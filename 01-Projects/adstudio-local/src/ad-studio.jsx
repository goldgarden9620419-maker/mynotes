...
/* ─────────────────────────────────────────────
   DESIGN TOKENS — "EDIT BAY"
   따뜻한 무채색 편집실 배경 + 본화이트 주동작 +
   씬마다 고유한 컬러바(SMPTE 유래)로 위치를 표시
────────────────────────────────────────────── */

const C = {
  base: "#131211",     // 편집실 벽 (따뜻한 근접 블랙)
  panel: "#1B1A18",    // 카드
  raised: "#232120",   // 슬레이트 헤더
  field: "#100F0E",    // 입력·프롬프트 바탕
  line: "#34302C",
  lineSoft: "#252220",
  text: "#F3EFE9",     // 본
  sub: "#A69E94",
  dim: "#6E665D",
  bone: "#F3EFE9",     // 주동작 (버튼)
  boneSoft: "rgba(243,239,233,0.09)",
  red: "#D4574A",      // 레코딩 · 오류
  ok: "#84A86D",
};
...
const disp = "'IBM Plex Sans Condensed', 'Pretendard', system-ui, sans-serif";
const body =
...
/* ───────── 로컬/아티팩트 환경 어댑터 ─────────
   claude.ai 아티팩트: window.storage 사용, API 키 불필요
   로컬(Claude Code + Vite): localStorage 폴백, API 키 필요        */

const IS_ARTIFACT = typeof window !== "undefined" && !!window.storage;

const store = {
  async get(key) {
    if (IS_ARTIFACT) return window.storage.get(key);
    const v = localStorage.getItem(key);
    return v == null ? null : { key, value: v };
  },
  async set(key, value) {
    if (IS_ARTIFACT) return window.storage.set(key, value);
    localStorage.setItem(key, value);
    return { key, value };
  },
  async delete(key) {
    if (IS_ARTIFACT) return window.storage.delete(key);
    localStorage.removeItem(key);
    return { key, deleted: true };
  },
};

const API_KEY_KEY = "adstudio:apikey";
function getApiKey() {
  try { return localStorage.getItem(API_KEY_KEY) || ""; } catch { return ""; }
}
function setApiKey(v) {
  try { v ? localStorage.setItem(API_KEY_KEY, v) : localStorage.removeItem(API_KEY_KEY); } catch {}
}
...
async function apiFetch(body) {
  ...
  if (!IS_ARTIFACT) {
    const key = getApiKey();
    if (!key)
      throw new Error("로컬 실행에는 Anthropic API 키가 필요합니다. 상단의 'API 키' 버튼으로 등록해주세요.");
...
function CopyBtn({ text, small, label = "복사" }) {
...
/* 한국어·영문 프롬프트를 탭 하나로 묶어 카드 높이를 절반으로 줄인다 */
function PromptPair({ label, kr, en, icon }) {
          ...
          {["KR", "EN"].map((l) => (
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                background: lang === l ? C.bone : "transparent",
                color: lang === l ? C.base : C.dim,
              }}
            >
              {l}
            </button>
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: "9px 6px",
              borderRadius: 4,
              border: `1px solid ${active ? C.bone : C.line}`,
              background: active ? C.boneSoft : C.field,
...
function Chip({ active, onClick, children, size = 11 }) {
  return (
    <button
      type="button"
      className="ad-tap"
      ...
      style={{
        ...
        borderRadius: 999,
        border: `1px solid ${active ? C.bone : C.line}`,
        background: active ? C.boneSoft : "transparent",
        color: active ? C.text : C.sub,
        cursor: "pointer",
      }}
    >
...
/* 컬러바 마크 — 로고이자 씬 식별 체계의 원형 */
function BarMark({ h = 20, w = 3, gap = 2, count = 6 }) {
  return (
    <div style={{ display: "flex", gap, height: h, alignItems: "stretch" }}>
      {BARS.slice(0, count).map((c) => (
        <span key={c} style={{ width: w, background: c, borderRadius: 1 }} />
      ))}
    </div>
  );
}
...
function DropZone({ onFile, drag, setDrag, title, hint, inputId }) {
  return (
    <label
      ...
      style={{
        display: "block",
        border: `1px dashed ${drag ? C.bone : C.line}`,
        borderRadius: 5,
        background: drag ? C.boneSoft : C.field,
        padding: "20px 12px",
        cursor: "pointer",
        color: C.sub,
        textAlign: "center",
    ...
    >
      ...
      <div style={{ fontSize: 10.5, color: C.dim, marginTop: 3 }}>{hint}</div>
    </label>
  );
}
...
export default function AdStudio() {
  ...
  /* ── 자동 저장: 브리프·씬 결과를 window.storage에 보존 (아티팩트 재로드에도 유지) ── */
  const STORAGE_KEY = "adstudio:v1";
        ...
        if (r?.value) {
          ...
          if (Array.isArray(s.scenes) && s.scenes.length) {
            setScenes(
              s.scenes.map((sc) =>
                sc.status === "loading"
                  ? { ...sc, status: "error", errMsg: "저장 시점에 생성이 끝나지 않은 씬입니다. 다시 생성을 눌러주세요." }
                  : sc
              )
            );
          }
          if (Array.isArray(s.selected)) setSelected(s.selected);
          if (s.multi && s.multi.status === "done") setMulti(s.multi);
          if (s.multiPicked) setMultiPicked(s.multiPicked);
          if (s.multiVid && s.multiVid.status === "done") setMultiVid(s.multiVid);
          if (s.multiImg) setMultiImg(s.multiImg);
          if (s.form?.product) setRestored(true);
        }
      } catch (e) {
        console.error("저장된 브리프 복원 실패:", e);
      } finally {
  ...
  useEffect(() => {
    if (!hydrated.current || running) return;
    const t = setTimeout(() => {
      store
        .set(
          STORAGE_KEY,
...
`;
  ...
  const fmtLabel = FORMATS.find((f) => f.id === form.format)?.label || "";
  const canGenerate = form.product.trim() && !running && !analyzing;
  const selStripe =
    selected.length >= 2
      ? `linear-gradient(90deg, ${selected
          .map(
            (i, n) =>
              `${barOf(i)} ${(n * 100) / selected.length}%, ${barOf(i)} ${
                ((n + 1) * 100) / selected.length
              }%`
          )
          .join(", ")})`
      : C.lineSoft;
  ...
  const goToScene = (i) => {
    document.getElementById(`scene-${i}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500&family=IBM+Plex+Sans+Condensed:wght@500;600;700&display=swap');

        * { -webkit-tap-highlight-color: transparent; }
        button { font-family: inherit; }
        input, textarea { font-family: ${body}; }
        input::placeholder, textarea::placeholder { color: ${C.dim}; }
        input:focus, textarea:focus { outline: none; border-color: ${C.bone} !important; }
        :focus-visible { outline: 2px solid ${C.bone}; outline-offset: 2px; }
        ::selection { background: ${C.bone}; color: ${C.base}; }

        .ad-tap { transition: border-color .15s, background .15s, color .15s; }
        .ad-tap:hover { border-color: ${C.sub}; }
        .ad-ghost { transition: border-color .15s, color .15s; }
        .ad-ghost:hover { border-color: ${C.sub}; color: ${C.text}; }
        .ad-primary { transition: transform .12s, background .15s, color .15s; }
        .ad-primary:not(:disabled):hover { transform: translateY(-1px); }
        .ad-primary:not(:disabled):active { transform: translateY(0); }
        .ad-seg:hover { filter: brightness(1.25); }

        @keyframes spin { to { transform: rotate(360deg); } }
        @keyframes rec { 50% { opacity: .18; } }
        @keyframes runner { to { background-position: 22px 0; } }

        ::-webkit-scrollbar { width: 10px; height: 10px; }
        ::-webkit-scrollbar-thumb { background: ${C.line}; border-radius: 10px; border: 3px solid ${C.base}; }
        ::-webkit-scrollbar-track { background: transparent; }

        input[type=range] { -webkit-appearance: none; appearance: none; height: 3px; background: ${C.line}; border-radius: 2px; }
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 14px; height: 14px; border-radius: 50%; background: ${C.bone}; border: none; cursor: pointer; }
        input[type=range]::-moz-range-thumb { width: 14px; height: 14px; border-radius: 50%; background: ${C.bone}; border: none; cursor: pointer; }

        .ad-grid { display: grid; grid-template-columns: 356px 1fr; gap: 18px; padding: 18px; max-width: 1280px; margin: 0 auto; }
        .ad-brief { position: sticky; top: 68px; max-height: calc(100vh - 86px); overflow-y: auto; }
        @media (max-width: 920px) {
          .ad-grid { grid-template-columns: 1fr; padding: 13px; gap: 13px; }
          .ad-brief { position: static; max-height: none; overflow: visible; }
        }
      ...
      >
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                const v = window.prompt(
                  "Anthropic API 키를 입력하세요 (브라우저 localStorage에만 저장됩니다. 비우고 확인하면 삭제):",
                  cur
                );
                if (v !== null) setApiKey(v.trim());
              }}
              title="로컬 실행용 Anthropic API 키 등록"
              style={{ ...S.ghostBtn, color: getApiKey() ? C.ok : C.red, borderColor: getApiKey() ? C.line : C.red }}
            >
              API 키{getApiKey() ? " ✓" : " 필요"}
            </button>
          )}
          ...
          {(form.product || scenes.length > 0) && !running && (
            <button
              type="button"
              className="ad-ghost"
              onClick={resetAll}
              title="브리프와 씬을 모두 지우고 새로 시작"
              style={{ ...S.ghostBtn }}
            >
              새 브리프
            </button>
          ...
          {doneCount > 0 && !running && <CopyBtn text={fullText()} label="제작안 전체 복사" />}
        </div>
      </header>
      ...
      <div className="ad-grid">
        ...
        >
          ...
          >
            <BarMark h={12} w={2} gap={2} count={3} />
            <span style={{ ...S.eyebrow, color: C.text, fontSize: 11.5 }}>Production Brief</span>
          </div>
          ...
          <div>
            ...
            >
              <span style={{ fontFamily: disp, fontSize: 30, fontWeight: 700, lineHeight: 1 }}>
                {form.duration}
                <span style={{ fontSize: 14, color: C.sub, marginLeft: 2 }}>초</span>
              </span>
              <span style={{ fontFamily: mono, fontSize: 11, color: C.dim }}>
                씬 {sceneCountFor(form.duration)}개
              </span>
              <div style={{ marginLeft: "auto", display: "flex", gap: 2, alignItems: "center" }}>
                {Array.from({ length: sceneCountFor(form.duration) }).map((_, i) => (
                  <span
                    key={i}
                    style={{ width: 12, height: 4, background: barOf(i), borderRadius: 1 }}
                  />
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <Seg
              ...
              render={(o, active) => (
                <>
                  <div style={{ fontFamily: mono, fontSize: 13, fontWeight: 500 }}>{o.label}</div>
                  <div style={{ fontSize: 10, color: active ? C.sub : C.dim, marginTop: 2 }}>
                    {o.sub}
                  </div>
                </>
              )}
          ...
          <div>
            ...
            <button
              type="button"
              className="ad-tap"
              aria-pressed={form.hasProductImage}
              ...
              style={{
                ...
                cursor: "pointer",
                textAlign: "left",
                border: `1px solid ${form.hasProductImage ? C.bone : C.line}`,
                background: form.hasProductImage ? C.boneSoft : C.field,
              }}
            >
              <span
                style={{
                  ...
                  borderRadius: 999,
                  position: "relative",
                  background: form.hasProductImage ? C.bone : C.line,
              ...
              >
                <span
                  style={{
                    ...
                    position: "absolute",
                    top: 2,
                    left: form.hasProductImage ? 16 : 2,
                    transition: "left .15s",
                  }}
                />
              </span>
              <span>
                <span
                  style={{
                    ...
                    fontSize: 12.5,
                    fontWeight: 600,
                    color: form.hasProductImage ? C.text : C.sub,
                  }}
                >
                  <Package size={13} /> 실제 제품 사진 쓰기
                </span>
                <span style={{ display: "block", fontSize: 10.5, color: C.dim, marginTop: 3 }}>
                  {form.hasProductImage
                    ? "사진을 올리면 분석해서 스타일에 반영합니다"
                    : "제품명만으로 외형을 프롬프트에 묘사합니다"}
                </span>
              </span>
            </button>
          </div>
              ...
              {!productImage ? (
                ...
                >
                  ...
                  {analyzeErr && (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="ad-ghost"
                        onClick={reAnalyze}
                        style={{ ...S.ghostBtn, color: C.red, borderColor: C.red, padding: "2px 8px" }}
                      >
                        다시 분석
                      </button>
                    </div>
                  ...
                  {analysis && (
                    ...
                    >
                      <div style={{ ...S.eyebrow, fontSize: 10, color: C.sub }}>
                        분석 결과 · {analysis.category}
                      </div>
                      <p style={{ fontSize: 11.5, lineHeight: 1.65, color: C.sub, margin: 0 }}>
                      ...
                      <div
                        style={{
                          borderLeft: `2px solid ${C.bone}`,
                          paddingLeft: 10,
                        }}
                      >
                        <div style={{ ...S.eyebrow, fontSize: 9.5, marginBottom: 3 }}>
                          추천 비주얼 스타일
                        </div>
                        <p style={{ fontSize: 11.5, lineHeight: 1.6, color: C.text, margin: 0 }}>
                          {analysis.style}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        onClick={reAnalyze}
                        disabled={analyzing}
                        style={{ ...S.ghostBtn, alignSelf: "flex-start", fontSize: 10.5 }}
                      >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 4,
              border: "none",
              background: canGenerate ? C.bone : C.raised,
              color: canGenerate ? C.base : C.dim,
              fontFamily: disp,
              fontSize: 15,
              fontWeight: 700,
              letterSpacing: "0.06em",
          ...
          >
            ...
            {running ? "생성 중" : analyzing ? "사진 분석 중" : "제작안 생성"}
          </button>
        </aside>
        ...
        <main style={{ display: "flex", flexDirection: "column", gap: 13, minWidth: 0 }}>
          {scenes.length === 0 ? (
            ...
            >
              ...
              >
                브리프를 채우면 여기에 씬이 깔립니다
              </div>
              <p style={{ fontSize: 13, color: C.sub, margin: "0 0 26px", lineHeight: 1.7 }}>
                씬마다 스토리보드 · 자막 카피 · 이미지/영상 프롬프트(한국어·영문)를 만들고,
                마음에 드는 씬을 골라 한 장으로 합칠 수 있습니다.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ["제품명과 소구점을 적습니다", "포맷·무드·길이·비율까지 고르면 준비 끝"],
                  ["씬 단위로 제작안이 생성됩니다", "앞 씬의 마지막 상태를 이어받아 한 이야기로 연결"],
                  ["씬을 2개 이상 골라 합칩니다", "합성 이미지 프롬프트 → 그 이미지로 영상 프롬프트"],
                ].map(([t, d], i) => (
                  <div key={t} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    />
                    <span>
                      <span style={{ display: "block", fontSize: 13, color: C.text }}>{t}</span>
            ...
            <>
              ...
              >
                <span style={{ color: C.text, fontFamily: body, fontWeight: 600, fontSize: 13 }}>
                  {form.product}
                </span>
                <span>{fmtLabel}</span>
                {form.mood && <span>{form.mood}</span>}
                <span>
                  {form.duration}s · {form.ratio}
                </span>
                <span>{form.hasProductImage ? "제품 사진 참조" : "사진 없음"}</span>
                <span style={{ marginLeft: "auto", color: C.sub }}>
                  {doneCount}/{scenes.length} 완료
                </span>
              </div>
              ...
              {arc && (
                ...
                >
                  <Film size={14} color={C.sub} style={{ flexShrink: 0, marginTop: 3 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ ...S.eyebrow, fontSize: 10, marginBottom: 5 }}>Story Concept</div>
                    <p style={{ fontSize: 13.5, lineHeight: 1.75, color: C.text, margin: 0 }}>
                      {arc.concept}
                    </p>
                  </div>
              ...
              <div style={{ ...S.card, padding: "12px 14px" }}>
                ...
                >
                  <span>Timeline · {form.duration}s</span>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: err
                            ...
                            : done
                            ? col
                            : `repeating-linear-gradient(115deg, ${C.line} 0 8px, ${C.lineSoft} 8px 16px)`,
                          animation: s.status === "loading" ? "runner .7s linear infinite" : "none",
                          opacity: done || err ? 1 : 0.8,
                          transition: "background .3s",
                          fontFamily: mono,
                          fontSize: 10,
                          color: done ? "rgba(0,0,0,0.62)" : C.dim,
                          fontWeight: 600,
                          overflow: "hidden",
                          minWidth: 0,
                        }}
              ...
              {scenes.map((s, i) => {
                ...
                return (
                  <section
                    ...
                    style={{
                      ...S.card,
                      borderColor: picked ? `${col}77` : C.lineSoft,
                      scrollMarginTop: 78,
                    }}
                  >
                    <div
                      style={{
                        height: 3,
                        background: s.status === "error" ? C.red : col,
                      ...
                      {s.status === "done" && (
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: 3,
                            flexShrink: 0,
                            border: `1.5px solid ${picked ? col : C.line}`,
                        ...
                        >
                          {picked && <Check size={11} color="#141210" strokeWidth={3} />}
                        </button>
                      )}
                      <span
                        style={{
                      ...
                      <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        {s.status === "error" && (
                          <button
                            ...
                            style={{ ...S.ghostBtn, color: C.red, borderColor: C.red }}
                          >
                        ...
                        {s.status === "done" && (
                          <button
                            ...
                            onClick={() => retryScene(i)}
                            title="이 씬 다시 생성"
                            style={{ ...S.ghostBtn, padding: "4px 7px", color: C.dim }}
                          >
                            <RefreshCw size={12} />
                          </button>
                        )}
                    ...
                    {s.status === "loading" && (
                      <div style={{ padding: "22px 16px", color: C.dim, fontSize: 12, fontFamily: mono }}>
                        씬 설계 중…
                      </div>
                    )}
                    ...
                    {s.status === "error" && (
                      <div style={{ padding: 18 }}>
                        ...
                        >
                          <AlertTriangle size={14} /> 이 씬을 만들지 못했습니다. 다시 생성을 눌러주세요.
                        </div>
                        {s.errMsg && (
                          <div
                            style={{
                    ...
                    {s.status === "done" && s.data && (
                      <div style={{ padding: 15, display: "flex", flexDirection: "column", gap: 13 }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontSize: 14.5, lineHeight: 1.8, color: C.text, margin: 0 }}>
                            {s.data.storyboard}
                          </p>
                        </div>
                        ...
                        >
                          <Type size={13} color={C.dim} style={{ flexShrink: 0 }} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ ...S.eyebrow, fontSize: 9.5, marginBottom: 3 }}>
                              자막 카피 · 후반 편집용
                            </div>
                        ...
                        >
                          <PromptPair
                            ...
                            kr={s.data.image_prompt_kr}
                            en={s.data.image_prompt_en}
                          />
                          <PromptPair
                            label="Video · 움직임"
                            icon={<Film size={11} />}
                            kr={s.data.video_prompt_kr}
                            en={s.data.video_prompt_en}
                          />
                        </div>
                      </div>
                    )}
              ...
              {doneCount >= 2 && (
                <section
                  style={{
                    ...S.card,
                    borderColor: selected.length >= 2 ? C.line : C.lineSoft,
                  }}
                >
                  <div style={{ height: 3, background: selStripe }} />
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    ...
                    <span style={{ fontSize: 11.5, color: C.dim, maxWidth: 520, lineHeight: 1.6 }}>
                      {selected.length >= 2
                        ? `씬 ${selected.map((i) => i + 1).join(" + ")}을 한 장으로 합칩니다`
                        : "씬 카드의 체크박스로 2개 이상 고르세요"}
                    </span>
                    <button
                      type="button"
                      className="ad-primary"
                      ...
                      style={{
                        ...
                        border: "none",
                        background:
                          selected.length >= 2 && multi?.status !== "loading" ? C.bone : C.panel,
                        color: selected.length >= 2 && multi?.status !== "loading" ? C.base : C.dim,
                        cursor:
                          selected.length >= 2 && multi?.status !== "loading" ? "pointer" : "default",
                      }}
                    >
                      ...
                      {multi?.status === "loading" ? "생성 중" : "합성 이미지 프롬프트 만들기"}
                    </button>
                  </div>

                  {multi?.status === "loading" && (
                    <div style={{ padding: 18, color: C.dim, fontSize: 12, fontFamily: mono }}>
                      선택한 씬을 한 장으로 합치는 프롬프트 생성 중… (KR·EN)
                    </div>
                  )}
                  ...
                  {multi?.status === "error" && (
                    <div style={{ padding: 18 }}>
                      ...
                      >
                        <AlertTriangle size={14} /> 합성 프롬프트를 만들지 못했습니다. 다시 시도해주세요.
                      </div>
                      <div
                        style={{
                          fontFamily: mono,
                    ...
                    <div style={{ padding: 15, display: "flex", flexDirection: "column", gap: 16 }}>
                      ...
                      >
                        <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                          ...
                          씬 {multi.data.sceneIdxs.map((i) => i + 1).join(" + ")} → 합성{" "}
                          {multi.data.shotCount}컷 · 영상 {multi.data.duration}s · {form.ratio}
                        </span>
                      ...
                      <div>
                        ...
                        >
                          <PromptPair
                            label="합성 Image"
                            icon={<ImageIcon size={11} />}
                            kr={multi.data.image_prompt_kr}
                            en={multi.data.image_prompt_en}
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        >
                          <span
                            style={{
                              ...
                              fontSize: 10,
                              fontWeight: 600,
                              color: multiVid?.status === "done" ? C.base : C.dim,
                              background: multiVid?.status === "done" ? C.bone : C.raised,
                              borderRadius: 3,
                              padding: "2px 7px",
                            }}
                          >
                        ...
                        <div
                          style={{ fontSize: 11, color: C.dim, marginBottom: 9, lineHeight: 1.6 }}
                        >
                          STEP 1 프롬프트로 만든 이미지를 올리면, 그 이미지에 맞춰 영상
                          프롬프트(한국어·영문)를 만듭니다.
                        </div>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div
                                style={{
                                  ...
                                  fontSize: 10,
                                  color:
                                    multiVid?.status === "loading"
                                      ? C.text
                                      : multiVid?.status === "done"
                                      ? C.ok
                                      : C.dim,
                                  marginTop: 3,
                                  display: "flex",
                          ...
                          <div style={{ marginTop: 9 }}>
                            ...
                            {multiImg && (
                              <button
                                ...
                                style={{ ...S.ghostBtn, marginTop: 7, fontSize: 10.5 }}
                              >
                                다시 올리기
                              </button>
                            )}
                        ...
                        {multiVid?.status === "done" && multiVid.data && (
                          ...
                          >
                            <PromptPair
                              label="영상화 Video"
                              icon={<Film size={11} />}
                              kr={multiVid.data.video_prompt_kr}
                              en={multiVid.data.video_prompt_en}
                            />
                          </div>
                        )}
                      </div>

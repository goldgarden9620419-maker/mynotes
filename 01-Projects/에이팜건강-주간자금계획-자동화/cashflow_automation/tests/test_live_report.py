...
def test_라이브양식_다음주로_재고정(tmp_path):
    ...
    daily = wb["4주일별계획"]
    # 6행 = 전일 마감(화요일) 참고 행, 7행 = 오늘(수요일) 실잔고 행
    assert daily["A6"].value.date() == NEW_MONDAY + timedelta(days=1)
    assert "전일 마감 실잔고" in str(daily["K6"].value)
    assert daily["A7"].value.date() == NEW_MONDAY + timedelta(days=2)
    assert "실잔고" in str(daily["K7"].value)
    # 날짜가 새 주차로 이동 (계획은 8행부터)
    assert daily["A8"].value.date() == NEW_MONDAY
    assert daily["A35"].value.date() == NEW_MONDAY + timedelta(days=27)
    # 반영률 수식은 그대로
    assert "INDEX" in daily["C8"].value and "$B$13" in daily["C8"].value
    # 지출 값 기록 (1일차 기타지출, 4일차 카드)
    assert daily["G8"].value == 100000
    assert daily["F11"].value == 500000
    ...
    assert not daily.row_dimensions[7].hidden
    assert daily.row_dimensions[8].hidden
    assert daily.row_dimensions[9].hidden
    assert not daily.row_dimensions[10].hidden
...
def test_실행일_행은_실적이어도_보인다(tmp_path):
    ...
    wb = load_workbook(out)
    daily = wb["4주일별계획"]
    assert daily.row_dimensions[8].hidden          # 월 (지난 실적)
    assert not daily.row_dimensions[9].hidden      # 화 (실행일 — 실적이어도 표시)
    assert not daily.row_dimensions[10].hidden     # 수 (예측)
    wb.close()
...
def test_실행일_차주금요일_붉은_상자(tmp_path):
    ...
    _assert_exec_box(wb["4주일별계획"], 8, 19)
    # 비고란은 자동 줄바꿈
    assert wb["4주일별계획"]["K10"].alignment.wrap_text
    wb.close()
    ...
    fill_live_workbook(out1, report2, out2)
    wb = load_workbook(out2)
    _assert_exec_box(wb["4주일별계획"], 11, 19)
    wb.close()
...
def test_계좌별시나리오_입금배분_표시(tmp_path):
    ...
    # 예측 행(10행=수요일)은 수식: 요약!B13(반영률)을 바꾸면 즉시 재계산
    # (계좌 2개 배치의 4주일별계획: E 온라인, H 확정, I 송금, J 카드, K 조정)
    assert ws.cell(row=10, column=6).value \
        == "='4주일별계획'!E10+'4주일별계획'!H10"          # ② 합계
    assert ws.cell(row=10, column=7).value == "=F10*0.700000"
    assert ws.cell(row=10, column=8).value == "=F10*0.300000"
    assert ws.cell(row=10, column=9).value \
        == "='4주일별계획'!I10+'4주일별계획'!J10+'4주일별계획'!K10"  # ③
    # 이체 계산용 숨김 열(M) + 표시 열(J=농협→우리)
    assert ws.column_dimensions["M"].hidden
    # 전날 잔액은 바로 윗 행(9행=마지막 실적) 셀을 참조한다
    assert ws.cell(row=10, column=13).value \
        == "=MIN(MAX(0,E9+H10),MAX(0,I10-D9-G10))"
    assert ws.cell(row=10, column=10).value == "=M10"
    assert ws.cell(row=10, column=11).value is None        # 국민 계좌 없음
    assert ws.cell(row=10, column=4).value == "=D9+G10-I10+M10"
    assert ws.cell(row=10, column=5).value == "=E9+H10-M10"
    assert ws.cell(row=10, column=3).value == "=SUM(D10:E10)"
    assert "부족" in str(ws.cell(row=10, column=12).value)  # 경고 수식
    # 일자·요일·총잔액·출발 행 2개 고정 + 수식 보호(암호 없음)
    ...
    # 6행 전일·7행 오늘 출발 행: 계좌별 실잔고(시나리오 같은 행 참조)
    assert daily.cell(row=6, column=3).value == "='계좌별시나리오'!D6"
    assert "전일 마감 실잔고" in str(daily.cell(row=6, column=15).value)
    assert daily.cell(row=7, column=3).value == "='계좌별시나리오'!D7"
    assert "실잔고" in str(daily.cell(row=7, column=15).value)
    assert daily.cell(row=10, column=3).value == "='계좌별시나리오'!D10"
    assert daily.cell(row=10, column=4).value == "='계좌별시나리오'!E10"
    assert daily.cell(row=10, column=6).value == "='계좌별시나리오'!G10"
    assert daily.cell(row=10, column=7).value == "='계좌별시나리오'!H10"
    ...
    assert daily.cell(row=10, column=12).value == "=E10+H10-I10-J10-K10"
    assert daily.cell(row=10, column=14).value == "=M10+L10"
    # 요약 4주 기말·최저 수식도 새 기말 열(N)·행(8~35)으로 재작성
    assert wb["요약"]["B14"].value == "='4주일별계획'!N35"
    # 13주 1주차 SUMIFS는 새 온라인 열(E)·행(8~35)을 합산
    assert "'4주일별계획'!$E$8:$E$35" in str(wb["13주주별계획"]["C6"].value)
    wb.close()

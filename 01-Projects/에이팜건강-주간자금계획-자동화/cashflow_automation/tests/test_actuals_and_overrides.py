...
def test_당일_행은_실제_입출금만으로_마감된다():
    ...
    # 실행일 행: 실제 값으로 마감. 출금 3,468,293은 거래처 이름 근거가
    # 없고 금액도 정확히 일치하지 않으므로 자동으로 집행 처리하지 않는다
    # (2026-09-22 — 무관한 출금이 계획에 흡수되는 오귀속 방지, 차이는
    # 확인 단계에서 사용자가 유지/보류로 결정). 계획외 지출로 마감되고
    # 기말 = 현재 실잔고는 그대로다.
    d21 = fc["daily"][0]
    assert round(d21["온라인 예상입금"]) == 10_373_905
    assert round(d21["확정·기타입금"]) == 20_110_000
    assert round(d21["팀별 송금예정"]) == 0            # 자동 집행 아님
    assert round(d21["기타지출"]) == 3_468_293         # 계획외 지출로 마감
    assert round(d21["기초잔액"]) == 161_670_332 - 27_015_612
    assert round(d21["기말잔액"]) == 161_670_332
    # 미집행 계획 3,498,000은 전액 익일로 이월 (보류 선택 시 제외 가능)
    assert round(fc["daily"][1]["팀별 송금예정"]) == 3_498_000

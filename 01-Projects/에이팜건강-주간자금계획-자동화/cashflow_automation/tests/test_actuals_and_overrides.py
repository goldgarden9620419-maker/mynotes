...
def test_실행일_당일_실제_입출금을_계획행에_합친다():
    """당일 행 = 이미 확인된 실적 + 남은 계획 (2026-09-21 사용자 요청).

    실사용 사례: 실행일에 이미 입금 30,483,905(온라인 10,373,905 +
    가지급금 회수 등 20,110,000)·출금 3,468,293(네이버 광고비
    2,178,000+330,000 분할 집행 + 계획외 960,293)이 있었는데
    계획 행에는 하나도 보이지 않았다.
    """
    base = date(2026, 9, 21)

    def _btx(in_amt=0, out_amt=0, cls="", memo=""):
        return {"거래일": base, "입금액": float(in_amt),
                "출금액": float(out_amt), "자동분류": cls,
                "내부이체": False, "기재내용·상대방": memo}

    history = [
        _tx(date(2026, 9, 1), in_amt=1_000_000, cls="온라인매출입금"),
        _btx(in_amt=10_373_905, cls="온라인매출입금", memo="스마트스토어정산"),
        _btx(in_amt=20_110_000, memo="가지급금 회수"),
        _btx(out_amt=2_178_000, memo="국민네이버 apha"),
        _btx(out_amt=330_000, memo="국민네이버 에이팜건"),
        _btx(out_amt=960_293, memo="통신요금 SKB6452342048"),
    ]
    plans = [{"자금계획 반영일": base, "예상금액": 3_498_000.0,
              "거래처": "네이버SA&GFA", "지급방법": "계좌송금",
              "확정여부": "확정"}]
    fc = fe.build_forecast(plans, base, 161_670_332, history, [], [],
                           [0.8], 0.8, run_date=base)
    d21 = fc["daily"][0]
    # 당일 행은 '남은 계획'만: 실제 입출금은 기초(은행 확인 실잔고)에 반영
    assert round(d21["기초잔액"]) == 161_670_332
    # 온라인: 실제(10,373,905)가 추정을 넘었으므로 남은 예상 0
    assert round(d21["온라인 예상입금"]) == 0
    # 계획에 없던 실제 입금은 실잔고에 이미 반영 → 계획 행엔 없음
    assert round(d21["확정·기타입금"]) == 0
    # 네이버 2건(2,508,000)은 계획 3,498,000 안의 집행 → 잔여만 표시
    assert round(d21["팀별 송금예정"]) == 3_498_000 - 2_508_000
    # 계획 밖 실제 출금(960,293)도 실잔고에 반영 → 계획 행엔 없음
    assert round(d21["기타지출"]) == 0
    assert "남은 계획만 표시" in d21["비고"]
    # 실제 입출금 집계는 출발 행 표시용으로 담겨 있다
    intraday = fc["intraday"]
    assert round(intraday["입금실제"]) == 30_483_905
    assert round(intraday["출금실제"]) == 3_468_293
    assert round(sum(intraday["집행"].values())) == 2_508_000
    assert round(intraday["계획외지출"]) == 960_293
    # 검산: 당일 기말 = 현재 실잔고 − 아직 안 나간 계획 지출(네이버 잔여)
    assert round(fc["start_balance"]) == 161_670_332 - (30_483_905
                                                        - 3_468_293)
    assert round(d21["기말잔액"]) == 161_670_332 - (3_498_000 - 2_508_000)


def test_당일_초과집행과_계획도착_입금은_이중계산하지_않는다():
    base = date(2026, 9, 21)
    history = [
        # 계획 1,000,000짜리를 실제로 1,200,000 집행 (초과 200,000)
        {"거래일": base, "입금액": 0.0, "출금액": 1_200_000.0,
         "자동분류": "", "내부이체": False, "기재내용·상대방": "엠제이유통"},
        # 확정입금 계획 500,000과 같은 금액의 실제 입금 → 계획 도착
        {"거래일": base, "입금액": 500_000.0, "출금액": 0.0,
         "자동분류": "", "내부이체": False, "기재내용·상대방": "정산입금"},
    ]
    plans = [{"자금계획 반영일": base, "예상금액": 1_000_000.0,
              "거래처": "엠제이유통", "지급방법": "계좌송금",
              "확정여부": "확정"}]
    adjustments = [{"일자": base, "조정입금": 500_000.0, "조정지출": 0.0,
                    "내용": "확정입금"}]
    fc = fe.build_forecast(plans, base, 10_000_000, history, adjustments,
                           [], [0.8], 0.8, run_date=base)
    d = fc["daily"][0]
    # 계획보다 많이 집행됨 → 남은 계획 0 (초과분은 실잔고에 이미 반영)
    assert round(d["팀별 송금예정"]) == 0
    # 확정입금 계획이 같은 금액으로 도착 → 남은 확정 0
    assert round(d["확정·기타입금"]) == 0
    assert round(d["기타지출"]) == 0
    # 기초 = 현재 실잔고, 기말 = 실잔고 + 남은 계획(전부 0)
    assert round(d["기초잔액"]) == 10_000_000
    assert round(d["기말잔액"]) == 10_000_000

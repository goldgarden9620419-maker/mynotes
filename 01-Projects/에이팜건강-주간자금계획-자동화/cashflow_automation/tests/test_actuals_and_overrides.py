...
def test_실행일_당일_실제_입출금을_계획행에_합친다():
    ...
    fc = fe.build_forecast(plans, base, 161_670_332, history, [], [],
                           [0.8], 0.8, run_date=base)
    d21 = fc["daily"][0]
    # 온라인: 실제 입금이 추정을 넘으면 실제를 쓴다
    assert round(d21["온라인 예상입금"]) == 10_373_905
    # 계획에 없던 실제 입금은 확정·기타입금으로
    assert round(d21["확정·기타입금"]) == 20_110_000
    # 네이버 2건(2,508,000)은 계획 3,498,000 안의 집행 → 계획 유지
    assert round(d21["팀별 송금예정"]) == 3_498_000
    # 계획과 매칭 안 된 실제 출금은 조정·추정 지출로
    assert round(d21["기타지출"]) == 960_293
    assert "당일 실적 반영" in d21["비고"]
...
def test_당일_초과집행과_계획도착_입금은_이중계산하지_않는다():
    ...
    fc = fe.build_forecast(plans, base, 10_000_000, history, adjustments,
                           [], [0.8], 0.8, run_date=base)
    d = fc["daily"][0]
    # 초과 집행분만 송금예정에 가산 (1,000,000 + 200,000)
    assert round(d["팀별 송금예정"]) == 1_200_000
    # 확정입금 계획이 도착한 것 → 실제 입금을 다시 더하지 않는다
    assert round(d["확정·기타입금"]) == 500_000
